...
//		 TODO: Implement moveOne
	...
	static int findShortestString(String[] paths, int lo, int hi) {
		...
		if (lo == hi) return lo;
		int indexOfShortest = findShortestString(paths, lo + 1, hi);
		if (safeStringLength(paths[lo]) <= safeStringLength(paths[indexOfShortest])) 
			return lo; 
		else 
	...
	public void chase(boolean maze[][], int targetX, int targetY) {
		...
		String result = shortestPath(x, y, targetX, targetY, maze);
		if (result != null && result != "") 
			moveOne(result.charAt(0));
	}
