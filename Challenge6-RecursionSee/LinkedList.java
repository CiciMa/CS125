...
public class LinkedList {
	...
	public String toString() {
		...
		// Recursive case:
		String restOfString = this.next.toString(); // Forward Recursion
		return this.word + ";" + restOfString;
		}
	}
	...
	public int getLetterCount() {
		if (next == null) {
			...
			return next.getLetterCount() + this.word.length();
		}
	...
	public String getLongestWord() {
		String longest = this.word; //store longest string in a variable
		if (next == null) {
			return this.word;
		} else {
			String tempLongest = this.next.getLongestWord();
	...
	* The last link represents the last word in the sentence.*/
	...
	public String getReversedSentence(String partialResult) {
		if (next == null) {
			...
			partialResult = next.getReversedSentence(" " + this.word + partialResult);
	...
	public LinkedList insert(String string) {
		LinkedList result;
		if (next == null) {
			result = new LinkedList(string,next);
			this.next = result;
			return this;
		}
		else {
				if(next.word.compareTo(string) > 0) {
					result = new LinkedList(string, this);
					return result;
				} else {
					result = next.insert(string);
				}
				return this;
		}
	}
